...
- Não adicione arquivos, campos, índices, configurações ou otimizações
  além do escopo pedido
- Não antecipe problemas futuros — deixe o TDD revelar o que é necessário
- Se perceber algo que pode ser necessário no futuro, mencione como
  observação mas não implemente
- Cada passo termina quando os testes do passo atual estão passando,
